const MCQ = (props) => {
    ...
    for ( var i = 0; i< props.choices.length; i++){
        rows.push(MCQRow(i, props.choices[i], props.name, props.handleChange));
    }
...
const MCQRow = (i, value, name, handleChange) => {
    return (
        <div><input className="my-3" type="radio" name={name} value={i} onChange={handleChange}/> <span>{value}</span> <br /></div>
